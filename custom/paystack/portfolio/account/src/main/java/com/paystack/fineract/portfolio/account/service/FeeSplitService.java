...
public class FeeSplitService {
    ...
    public void processFeeSplit(ClientTransaction clientTransaction, BigDecimal totalFeeAmount) {
        log.info("Processing fee split for client transaction: {}, amount: {}", 
                clientTransaction.getId(), totalFeeAmount);
        ...
        log.info("Fee split processing completed for transaction: {}, total splits: {}", 
                clientTransaction.getId(), splits.size());
    }
    ...
    public void processFeeSplitForSavings(SavingsAccountTransaction savingsTransaction, BigDecimal totalFeeAmount) {
        ...
        log.info("Transaction ID: {}, Amount: {}, Type: {}", 
                savingsTransaction.getId(), totalFeeAmount, savingsTransaction.getTransactionType());
        log.info("Transaction Date: {}, Currency: {}", 
                savingsTransaction.getTransactionDate(), savingsTransaction.getCurrency());
        ...
        log.info("✅ Charge found: ID={}, Name={}, Enable Fee Split={}", 
                charge.getId(), charge.getName(), charge.isEnableFeeSplit());
        ...
        // Log split details
        for (ChargeSplit split : splits) {
            log.info("Split: ID={}, Fund={}, Type={}, Value={}", 
                    split.getId(), split.getFund().getName(), 
                    split.isPercentageSplit() ? "PERCENTAGE" : "FLAT_AMOUNT", 
                    split.getSplitValue());
        }
        ...
        log.info("Fee split processing completed for savings transaction: {}, total splits: {}", 
                savingsTransaction.getId(), splits.size());
    }
    ...
    private Charge getChargeFromClientTransaction(ClientTransaction clientTransaction) {
        ...
        if (clientTransaction.getClientChargePaidByCollection() != null && 
            !clientTransaction.getClientChargePaidByCollection().isEmpty()) {
        ...
        try {
            ...
            if (savingsTransaction.getSavingsAccount() != null) {
                ...
                for (SavingsAccountCharge accountCharge : savingsTransaction.getSavingsAccount().charges()) {
                    ...
                    log.info("Checking charge: ID={}, Amount={}, Outstanding={}, Active={}", 
                            accountCharge.getCharge().getId(), 
                            chargeAmount, 
                            chargeOutstanding,
                            accountCharge.isActive());
                    
                    // Fix: Use type-safe comparison for Money vs BigDecimal
                    if (chargeAmount.compareTo(savingsTransaction.getAmount()) == 0) {
                        log.info("✅ Found matching charge: {} for transaction: {}", 
                                accountCharge.getCharge().getId(), savingsTransaction.getId());
                        log.info("Charge amount: {}, Transaction amount: {}, Match: {}", 
                                chargeAmount, savingsTransaction.getAmount(), chargeAmount.compareTo(savingsTransaction.getAmount()) == 0);
                        return accountCharge.getCharge();
                    } else {
                        log.debug("No match: Charge amount {} != Transaction amount {}", 
                                chargeAmount, savingsTransaction.getAmount());
                    }
                }
    ...
    private FeeSplitAudit createAuditRecordForSavings(SavingsAccountTransaction savingsTransaction, Charge charge, BigDecimal totalFeeAmount) {
    ...
    private void processIndividualSplit(ChargeSplit split, BigDecimal totalFeeAmount, 
                                      FeeSplitAudit audit, ClientTransaction clientTransaction) {
        try {
        ...
        BigDecimal splitAmount = split.calculateSplitAmount(totalFeeAmount);
            ...
            if (charge == null) {
                ...
                dataValidationErrors.add(ApiParameterError.parameterError(
                    "error.msg.fee.split.charge.not.found",
                    "Charge not found", "clientTransactionId", clientTransaction.getId()));
                
                throw new PlatformApiDataValidationException(
                    "error.msg.fee.split.charge.not.found",
                    "Charge not found for client transaction: " + clientTransaction.getId(),
                    dataValidationErrors);
            }
            
            // Create balanced journal entries
            List<JournalEntry> journalEntries = createJournalEntriesForSplit(
                split, splitAmount, clientTransaction, charge);
            
                        // Use the CREDIT entry for the split detail (maintains existing logic)
            JournalEntry creditEntry = journalEntries.stream()
                .filter(je -> je.getType().equals(JournalEntryType.CREDIT.getValue()))
                .findFirst()
                .orElseThrow(() -> new IllegalStateException("Credit entry not found in balanced journal entries"));
        
        // Create detail record
        FeeSplitDetail detail = FeeSplitDetail.createNew(audit, split.getFund(), splitAmount, 
                split.isPercentageSplit() ? split.getSplitValue() : null, 
                    split.getGlAccount(), creditEntry);
    ...
    private void processIndividualSplitForSavings(ChargeSplit split, BigDecimal totalFeeAmount, 
                                                FeeSplitAudit audit, SavingsAccountTransaction savingsTransaction) {
        log.info("Processing individual split: ID={}, Fund={}, Value={}", 
                split.getId(), split.getFund().getName(), split.getSplitValue());
        ...
        BigDecimal splitAmount = split.calculateSplitAmount(totalFeeAmount);
            ...
            if (charge == null) {
                ...
                dataValidationErrors.add(ApiParameterError.parameterError(
                    "error.msg.fee.split.charge.not.found",
                    "Charge not found", "savingsTransactionId", savingsTransaction.getId()));
                
                throw new PlatformApiDataValidationException(
                    "error.msg.fee.split.charge.not.found",
                    "Charge not found for savings transaction: " + savingsTransaction.getId(),
                    dataValidationErrors);
            }
            
            // Create balanced journal entries
            log.info("Creating balanced journal entries for split...");
            List<JournalEntry> journalEntries = createJournalEntriesForSavingsSplit(
                split, splitAmount, savingsTransaction, charge);
            ...
            log.info("✅ Journal entries created: DEBIT={}, CREDIT={}", 
                    journalEntries.stream()
                        .filter(je -> je.getType().equals(JournalEntryType.DEBIT.getValue()))
                        .findFirst().map(JournalEntry::getId).orElse(null),
                    creditEntry.getId());
    ...
    private List<JournalEntry> createJournalEntriesForSplit(ChargeSplit split, BigDecimal splitAmount, 
                                                     ClientTransaction clientTransaction, Charge charge) {
        log.info("Creating balanced journal entries for client split: Split ID={}, Amount={}", 
                split.getId(), splitAmount);
        ...
        String transactionId = generateTransactionId(office.getId());
            ...
            log.info("Office: {}, Currency: {}, Date: {}, Transaction ID: {}", 
                    office.getId(), currencyCode, transactionDate, transactionId);
            log.info("Original Income Account: {}, Destination GL Account: {}, Fund: {}", 
                    originalIncomeAccount.getId(), split.getGlAccount().getId(), split.getFund().getName());
            
            // Create DEBIT entry from original income account
            JournalEntry debitEntry = JournalEntry.createNew(
                    office,
                    null, // paymentDetail
                    originalIncomeAccount,
                    currencyCode,
                    transactionId,
                    false, // manualEntry
                    transactionDate,
                    ...
                    splitAmount,
                    "Fee split debit for " + split.getFund().getName(),
                    null, // entityType
                    null, // entityId
        ...
        JournalEntry creditEntry = JournalEntry.createNew(
                office,
                null, // paymentDetail
                split.getGlAccount(),
                currencyCode,
                ...
                splitAmount,
                "Fee split for " + split.getFund().getName(),
                null, // entityType
                null, // entityId
                null, // referenceNumber
                null, // loanTransactionId
            ...
            log.info("✅ Balanced journal entries created for client: DEBIT={}, CREDIT={}", 
                    persistedDebit.getId(), persistedCredit.getId());
    ...
    private List<JournalEntry> createJournalEntriesForSavingsSplit(ChargeSplit split, BigDecimal splitAmount, 
        ...
        log.info("Creating balanced journal entries for savings split: Split ID={}, Amount={}", 
                split.getId(), splitAmount);
        ...
        JournalEntry creditEntry = JournalEntry.createNew(
            ...
            log.info("✅ Balanced journal entries created: DEBIT={}, CREDIT={}", 
                    persistedDebit.getId(), persistedCredit.getId());
    ...
    private GLAccount getOriginalIncomeAccount(Charge charge, SavingsAccountTransaction savingsTransaction) {
        try {
            ...
            log.info("Getting original income account for charge {} from savings product {} using account type {}", 
                    chargeId, savingsProductId, accountTypeId);
            
            // 1. Get product-level default mapping (EXACT same as Fineract)
            ProductToGLAccountMapping accountMapping = accountMappingRepository
                .findCoreProductToFinAccountMapping(savingsProductId, PortfolioProductType.SAVING.getValue(), accountTypeId);
            ...
            if (accountTypeId == CashAccountsForSavings.INCOME_FROM_FEES.getValue()) {
                ...
                if (glAccount != null) {
                    log.info("✅ Using charge-specific account: ID={}, Name={}", 
                            glAccount.getId(), glAccount.getName());
                    return glAccount;
                }
                ...
                final ProductToGLAccountMapping chargeSpecificIncomeAccountMapping = accountMappingRepository
                    .findProductIdAndProductTypeAndFinancialAccountTypeAndChargeId(
                        savingsProductId, PortfolioProductType.SAVING.getValue(), accountTypeId, chargeId);
                if (chargeSpecificIncomeAccountMapping != null) {
                    accountMapping = chargeSpecificIncomeAccountMapping;
                    log.info("✅ Using charge-specific product mapping: ID={}, Name={}", 
                            accountMapping.getGlAccount().getId(), accountMapping.getGlAccount().getName());
                }
            }
            
            // 3. Return the best available account (EXACT same as Fineract)
            GLAccount finalAccount = accountMapping.getGlAccount();
            log.info("✅ Using product default account: ID={}, Name={}", 
                    finalAccount.getId(), finalAccount.getName());
            return finalAccount;
            ...
            throw new PlatformApiDataValidationException(
                List.of(ApiParameterError.parameterError(
                    "error.msg.fee.split.income.account.not.found",
                    "Original income account not found for charge: " + charge.getId(),
    ...
    private GLAccount getOriginalIncomeAccountForClient(Charge charge, ClientTransaction clientTransaction) {
        try {
            ...
            if (chargeAccount != null) {
                log.info("✅ Using charge's own account as income account: ID={}, Name={}", 
                        chargeAccount.getId(), chargeAccount.getName());
            ...
            throw new PlatformApiDataValidationException(
                List.of(ApiParameterError.parameterError(
                    "error.msg.fee.split.income.account.not.found",
                    "No income account found for client charge: " + charge.getId(),
                    "chargeId", charge.getId()
                ))
            );
            ...
            throw new PlatformApiDataValidationException(
                List.of(ApiParameterError.parameterError(
                    "error.msg.fee.split.income.account.not.found",
                    "Original income account not found for client charge: " + charge.getId(),
                    "chargeId", charge.getId()
                ))
            );
        }
    ...
    private void validateJournalEntryBalance(List<JournalEntry> journalEntries, BigDecimal expectedAmount) {
        ...
        if (totalDebits.compareTo(totalCredits) != 0) {
            throw new PlatformApiDataValidationException(
                "error.msg.fee.split.journal.entries.unbalanced",
                "Journal entries are not balanced: DEBITS={}, CREDITS={}", 
                List.of(ApiParameterError.parameterError(
                    "error.msg.fee.split.journal.entries.unbalanced",
                    "Journal entries unbalanced", "debits", totalDebits, "credits", totalCredits)));
        }
        
        if (totalDebits.compareTo(expectedAmount) != 0) {
            throw new PlatformApiDataValidationException(
                "error.msg.fee.split.journal.entries.amount.mismatch",
                "Journal entry amounts don't match expected: EXPECTED={}, ACTUAL={}", 
                List.of(ApiParameterError.parameterError(
                    "error.msg.fee.split.journal.entries.amount.mismatch",
                    "Amount mismatch", "expected", expectedAmount, "actual", totalDebits)));
        }
        
        log.info("✅ Journal entry balance validation passed: DEBITS={}, CREDITS={}, EXPECTED={}", 
                totalDebits, totalCredits, expectedAmount);
    }
    ...
    private void validateTransactionIdUniqueness(List<JournalEntry> journalEntries) {
        Set<String> transactionIds = journalEntries.stream()
        ...
        if (transactionIds.size() != 1) {
            List<ApiParameterError> dataValidationErrors = new ArrayList<>();
            dataValidationErrors.add(ApiParameterError.parameterError(
                "error.msg.fee.split.transaction.id.not.unique",
                "Transaction IDs not unique", "transactionIds", transactionIds));
            
            throw new PlatformApiDataValidationException(
                "error.msg.fee.split.transaction.id.not.unique",
                "Transaction IDs are not unique across journal entries: " + transactionIds,
                dataValidationErrors);
        }
     ...
     */
    private void logFeeSplitSummary(FeeSplitAudit audit, List<ChargeSplit> splits, 
        ...
        if (audit.getTotalFeeAmount().compareTo(totalSplitAmount) != 0) {
            log.warn("⚠️ WARNING: Total fee amount ({}) != Total split amount ({})", 
                    audit.getTotalFeeAmount(), totalSplitAmount);
        } else {
        ...
        for (FeeSplitDetail detail : audit.getSplitDetails()) {
            log.info("Split Detail: Fund={}, Amount={}, Percentage={}, GL Account={}, Journal Entry={}", 
                    detail.getFund().getName(), detail.getSplitAmount(), 
                    detail.getSplitPercentage(), detail.getGlAccount().getName(), 
