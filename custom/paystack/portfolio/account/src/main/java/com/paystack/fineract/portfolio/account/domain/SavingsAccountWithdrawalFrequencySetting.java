...
/**
 * Savings Account Withdrawal Frequency Setting Entity Represents withdrawal frequency limits configured at the account
 * level These settings override product-level settings for the same time period Each account can have multiple settings
 * for different time periods (daily, weekly, monthly, yearly)
 */
@Entity
@Table(name = "m_savings_account_withdrawal_frequency_setting", uniqueConstraints = @UniqueConstraint(columnNames = { "savings_account_id",
        "time_period" }, name = "uq_sawfs_account_period"))
@Getter
@Setter
@NoArgsConstructor
@AllArgsConstructor
public class SavingsAccountWithdrawalFrequencySetting extends AbstractAuditableWithUTCDateTimeCustom<Long> {
    ...
    public String toString() {
        return "SavingsAccountWithdrawalFrequencySetting{" + "id=" + getId() + ", savingsAccountId=" + savingsAccountId
                + ", maxWithdrawals=" + maxWithdrawals + ", timePeriod=" + timePeriod + ", isActive=" + isActive + '}';
    }
}
