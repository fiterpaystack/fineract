...
public class PaystackChargeData implements Serializable {
    ...
    public static PaystackChargeData fromChargeData(ChargeData chargeData, Map<String, Object> additionalAttributes) {
        return PaystackChargeData.builder().id(chargeData.getId()).name(chargeData.getName()).active(chargeData.isActive())
                .penalty(chargeData.isPenalty()).freeWithdrawal(chargeData.isFreeWithdrawal()).isPaymentType(chargeData.isIsPaymentType())
                .enableFeeSplit(chargeData.isEnableFeeSplit()).freeWithdrawalChargeFrequency(chargeData.getFreeWithdrawalChargeFrequency())
                .restartFrequency(chargeData.getRestartFrequency()).restartFrequencyEnum(chargeData.getRestartFrequencyEnum())
                .paymentTypeOptions(chargeData.getPaymentTypeOptions()).currency(chargeData.getCurrency()).amount(chargeData.getAmount())
                .chargeTimeType(chargeData.getChargeTimeType()).chargeAppliesTo(chargeData.getChargeAppliesTo())
                .chargeCalculationType(chargeData.getChargeCalculationType()).chargePaymentMode(chargeData.getChargePaymentMode())
                .feeOnMonthDay(chargeData.getFeeOnMonthDay()).feeInterval(chargeData.getFeeInterval()).minCap(chargeData.getMinCap())
                .maxCap(chargeData.getMaxCap()).feeFrequency(chargeData.getFeeFrequency())
                .incomeOrLiabilityAccount(chargeData.getIncomeOrLiabilityAccount()).taxGroup(chargeData.getTaxGroup())
                .currencyOptions(chargeData.getCurrencyOptions()).chargeCalculationTypeOptions(chargeData.getChargeCalculationTypeOptions())
                .chargeAppliesToOptions(chargeData.getChargeAppliesToOptions()).chargeTimeTypeOptions(chargeData.getChargeTimeTypeOptions())
                ...
                .expenseAccountOptions(chargeData.getExpenseAccountOptions()).assetAccountOptions(chargeData.getAssetAccountOptions())
                .varyAmounts(chargeData.getVaryAmounts()).chargeSlabs(chargeData.getChargeSlabs())
                .additionalAttributes(additionalAttributes).build();
    }
    ...
    public ChargeData toChargeData() {
        return ChargeData.builder().id(this.id).name(this.name).active(this.active).penalty(this.penalty)
                .freeWithdrawal(this.freeWithdrawal).isPaymentType(this.isPaymentType).enableFeeSplit(this.enableFeeSplit)
                .freeWithdrawalChargeFrequency(this.freeWithdrawalChargeFrequency).restartFrequency(this.restartFrequency)
                .restartFrequencyEnum(this.restartFrequencyEnum).paymentTypeOptions(this.paymentTypeOptions).currency(this.currency)
                .amount(this.amount).chargeTimeType(this.chargeTimeType).chargeAppliesTo(this.chargeAppliesTo)
                .chargeCalculationType(this.chargeCalculationType).chargePaymentMode(this.chargePaymentMode)
                .feeOnMonthDay(this.feeOnMonthDay).feeInterval(this.feeInterval).minCap(this.minCap).maxCap(this.maxCap)
                .feeFrequency(this.feeFrequency).incomeOrLiabilityAccount(this.incomeOrLiabilityAccount).taxGroup(this.taxGroup)
                .currencyOptions(this.currencyOptions).chargeCalculationTypeOptions(this.chargeCalculationTypeOptions)
                .chargeAppliesToOptions(this.chargeAppliesToOptions).chargeTimeTypeOptions(this.chargeTimeTypeOptions)
                ...
                .shareChargeTimeTypeOptions(this.shareChargeTimeTypeOptions).feeFrequencyOptions(this.feeFrequencyOptions)
                .incomeOrLiabilityAccountOptions(this.incomeOrLiabilityAccountOptions).taxGroupOptions(this.taxGroupOptions)
                .accountMappingForChargeConfig(this.accountMappingForChargeConfig).expenseAccountOptions(this.expenseAccountOptions)
                .assetAccountOptions(this.assetAccountOptions).varyAmounts(this.varyAmounts).chargeSlabs(this.chargeSlabs)
                .additionalAttributes(this.additionalAttributes).build();
    }
}
