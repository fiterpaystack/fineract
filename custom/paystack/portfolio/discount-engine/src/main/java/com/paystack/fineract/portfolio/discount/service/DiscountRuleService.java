...
/**
 * Consolidated Discount Rule Service following core Fineract patterns.
 * Handles all discount rule operations including CRUD and assignments.
 */
@Service
...
public class DiscountRuleService {
    ...
    public void assignDiscountRulesToCharge(Long chargeId, List<Long> ruleIds) {
        ...
        List<DiscountRule> rules = ruleIds.stream()
                .map(repositoryWrapper::findOneWithNotFoundDetection)
    ...
    public void removeAllDiscountRulesFromCharge(Long chargeId) {
        ...
        int removedCount = 0;
        List<Long> removedRuleIds = new ArrayList<>();
    ...
    private BigDecimal calculateDiscountWithRule(DiscountRule rule, BigDecimal originalAmount, DiscountContext context) {
        ...
        if (rule.getRuleType() != null && rule.getRuleParametersJson() != null) {
            try {
                ...
                log.warn("Failed to use calculator for rule {}: {}, falling back to legacy calculation", 
